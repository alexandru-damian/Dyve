...
namespace Intermediate {
	...
	BitmapSource^ Mapper::ProcessVideoFrame()
	{		
		cv::Mat image = _processer->ProcessVideo();
		return ToBitmapSource(image);
	}
	BitmapSource^ Mapper::ProcessAndSaveImage(String^ path, String^ savetopath)
	{
		char * pathchar = MapToCPPArrayChar(path);
		char * savetopathchar = MapToCPPArrayChar(savetopath);

		Processer processer;
		cv::Mat image = processer.ProcessAndSaveImage(pathchar, savetopathchar);
		return ToBitmapSource(image);
	}
